...
(def unicode-range-no-digits
  [[(char->int \A) (char->int \Z)]
   [(char->int \a) (char->int \z)]
   [0x0060 0x007B] ; * Latin letters w/ diacritics (excl. multiplication and
   [0x00C0 0x00D6] ;   division signs) + spacing modifiers
   [0x00D8 0x00F6] ;
   [0x0370 0x037D] ; * Various alphabets (excl. Greek question mark)
   [0x037F 0x1FFF] ;
...
(defn- ranges->regex-charset
  ...
  (let [charset
        (reduce (fn [s r]
                  (if (vector? r)
                    (let [[start end] r
                          start-str (num->unicode-str start)
                          end-str   (num->unicode-str end)]
                      (str s start-str "-" end-str))
                    (let [range-str (num->unicode-str r)]
                      (str s range-str))))
                ""
                ranges)]
    (str "[" charset "]")))
...
#?(:clj
   ...
   (defn- valid-var-str?*
     [^String var-str]
     (let [ccnt (.codePointCount var-str 0 (count var-str))]
       (loop [idx 0]
         (cond
           (>= idx ccnt)
           (<= 2 ccnt) ; Need to have initial qmark + at least one start char
           (= idx 0)
           (recur-if (in-bitset? qmark-bitset var-str idx)
                     (inc idx))
           (= idx 1)
           (recur-if (in-bitset? var-start-bitset var-str idx)
                     (inc idx))
           :else
           (recur-if (in-bitset? var-body-bitset var-str idx)
                     (inc idx)))))))
...
#?(:clj
   ...
   (defn- valid-bnode-str?*
     [^String bnode-str]
     (let [ccnt (.codePointCount bnode-str 0 (count bnode-str))
           lidx (dec ccnt)]
       (loop [idx 0]
         (cond
           (>= idx ccnt)
           ...
           (= idx 0)
           (recur-if (in-bitset? uscore-bitset bnode-str idx)
                     (inc idx))
           (= idx 1)
           (recur-if (in-bitset? bnode-start-bitset bnode-str idx)
                     (inc idx))
           (= idx lidx)
           (recur-if (in-bitset? bnode-end-bitset bnode-str idx)
                     (inc idx))
           :else
           (recur-if (in-bitset? bnode-body-bitset bnode-str idx)
                     (inc idx)))))))

#?(:clj
   #_{:clj-kondo/ignore [:loop-without-recur]}
   (defn- valid-prefix-ns-str?*
     [^String prefix-ns-str]
     (let [ccnt (.codePointCount prefix-ns-str 0 (count prefix-ns-str))
           lidx (dec ccnt)]
       (loop [idx 0]
         (cond
           (>= idx ccnt)
           ...
           (= idx 0)
           (recur-if (in-bitset? prefix-ns-start-bitset prefix-ns-str idx)
                     (inc idx))
           (= idx lidx)
           (recur-if (in-bitset? prefix-ns-end-bitset prefix-ns-str idx)
                     (inc idx))
           :else
           (recur-if (in-bitset? prefix-ns-body-bitset prefix-ns-str idx)
                     (inc idx)))))))
...
#?(:clj
   ...
   (defn- valid-prefix-name-str?*
     ...
     (let [ccnt (.codePointCount prefix-name-str 0 (count prefix-name-str))
           lidx (dec ccnt)]
       (loop [idx 0]
         (cond
           (>= idx ccnt)
           (<= 1 ccnt) ; Need to have at least one char
           (= idx 0)
           (recur-if (in-bitset? prefix-name-start-bitset prefix-name-str idx)
                     (inc idx))
           (= idx lidx)
           (recur-if (in-bitset? prefix-name-end-bitset prefix-name-str idx)
                     (inc idx))
           :else
           (recur-if (in-bitset? prefix-name-body-bitset prefix-name-str idx)
                     (inc idx)))))))
